...
/*   Updated: 2023/12/15 10:49:26 by rmarceau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	is_valid_identifier(char **args)
{
	...
	{
		if (args[i][0] == '=')
			return (print_error_builtin(ERR_INVALID_ID, args[0], args[i], 1),
				false);
		...
		while (args[i][++j] != '\0')
		{
			...
			if (ft_isalnum(args[i][j]) == 0 && args[i][j] != '_')
				return (print_error_builtin(ERR_INVALID_ID, args[0], args[i],
						1), false);
		}
	}
	return (true);
