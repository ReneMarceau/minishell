...
/*   Updated: 2023/12/15 11:24:41 by rmarceau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	handle_output_redirection(t_shell *shell, t_rdir *rdir)
{
	...
	{
		shell->output_fd = open(rdir->value, O_WRONLY | O_CREAT | O_TRUNC,
				0644);
	...
	{
		shell->output_fd = open(rdir->value, O_WRONLY | O_CREAT | O_APPEND,
				0644);
		if (handle_open_failed(shell->output_fd, rdir->value) == false)
			return (false);
...
bool	handle_pipe_redirections(t_shell *shell, t_cmd *cmd)
{
	if (cmd->index > 0 && has_redirection(cmd->rdir, REDIR_IN) == false
		&& has_redirection(cmd->rdir, HEREDOC) == false)
	{
		if (dup2(shell->pipe_fd[cmd->index - 1][READ_END], STDIN_FILENO) == -1)
			return (print_error(ERR_DUP2, NULL, EXIT_FAILURE), false);
	}
	if (cmd->next != NULL && has_redirection(cmd->rdir, REDIR_OUT) == false
		&& has_redirection(cmd->rdir, REDIR_APPEND) == false)
	{
...
bool	redirections_operation(t_shell *shell, t_rdir *rdir, char *heredoc_file)
{
	while (rdir != NULL)
	{
